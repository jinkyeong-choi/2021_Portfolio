...
> ### 사전조사<br>
**1. C++/C#간 이미지 송수신**<br>
-SNS에서 가장 중요한건 게시글을 작성하는 기능인데, 이 때 게시글에 이미지가 들어가야 매력있다고 생각한다. 그래서 서버와 클라이언트가 이미지를 주고 받는 방법을 찾아봤다.<br>
**2. 이미지 가져오기**<br>
-컴퓨터에 저장된 이미지를 가져와 컨트롤에 저장한 뒤 서버에 전송해야한다.<br>
-저장된 이미지를 가져오는 방법을 알아봤다.<br><br>

> ### 소감
**1. 어려웠던 점**<br>
-C++과 C# 간 이미지 송수신에서 계속 오류가 발생했다. 정해진 기간 내에 프로젝트를 끝내지 못할 것 같아 서버를 C++에서 C#으로 변경했다.<br>
**2. 알게된 점**<br>
-일반 문자 데이터는 C++과 C# 간 통신이 원활한데, 이미지 전송은 원하는 대로 되지 않는다.<br>
-다음에는 C++과 C# 간 이미지 송수신을 다시 구현해볼 계획이다.<br><br><br>
